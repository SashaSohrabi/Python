...
from network import hole_externe_ip
...
def zeige_externe_ip() -> None:
    ip = hole_externe_ip()
    if ip is None:
        return
    schreibe_log(LogLevel.INFO, f"Externe IP-Adresse: {ip}", Farbe.GRUEN)
...
def starte_menue() -> None:
    ...
    try:
        while True:
            ...
            print(f"{Farbe.BLAU}8 - Externe IP-Adresse anzeigen{Farbe.RESET}")
